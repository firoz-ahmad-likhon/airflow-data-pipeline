...
class TestIntegrationGX:
    ...
    def test_doc_site_added(
        self,
        gxi: type[GXInitiator],
        context: AbstractDataContext,
    ) -> None:
        """Assert that the doc site is added to the context."""
        assert (
            gxi.INGESTION_TIME_SITE_NAME in context.list_data_docs_sites()
        ), "Ingestion time site should exist in data docs sites"

    def test_validation_results_store_configured(
        self,
        context: AbstractDataContext,
    ) -> None:
        ...
        ), "Validation results store should be configured"

    def test_data_sources_added(
        self,
        gxi: type[GXInitiator],
        context: AbstractDataContext,
    ) -> None:
        ...
        assert (
            gxi.SOURCE_NAME in context.data_sources.all()
        ), "Data source should be added to the context"

    def test_data_assets_added(
        self,
        gxi: type[GXInitiator],
        context: AbstractDataContext,
    ) -> None:
        ...
        assert {
            gxi.SOURCE_NAME: [gxi.ASSET_NAME],
        } == context.get_available_data_asset_names(
            datasource_names=[gxi.SOURCE_NAME],
        ), "Data asset should be added to the data source"
    ...
    def test_validation_definitions(
        self,
        context: AbstractDataContext,
        dimensions: list[str],
    ) -> None:
    ...
    def test_checkpoints(
        ...
        for checkpoint in checkpoints:
            assert (
                checkpoint in gx_checkpoints
            ), f"Checkpoint '{checkpoint}' should exist"
